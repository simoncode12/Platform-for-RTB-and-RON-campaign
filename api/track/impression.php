...
try {
    ...
    // Log impression
    $stmt = $pdo->prepare("INSERT INTO impressions (zone_id, campaign_id, campaign_type, ip_address, user_agent, created_at) VALUES (?, ?, ?, ?, ?, NOW())");
    $stmt->execute([
        $zone['id'],
        $campaign_id,
        $type,
    ...
    echo json_encode(['success' => true, 'tracked' => 'impression']);
    
} catch (Exception $e) {
    error_log("Impression Tracking Error: " . $e->getMessage());
    ...
    try {
        ...
        if ($type === 'impression') {
            $stmt = $pdo->prepare("UPDATE zones SET 
                                   ...
                                   impressions_total = impressions_total + 1
                                   WHERE id = ?");
            $stmt->execute([$zone_id]);
        }
    } catch (Exception $e) {
...
try {
    ...
    error_log("Create impressions table error: " . $e->getMessage());
}
?>